...
function handleUpdateModelWhenEdit() {
  ...
  if (props.operateType === 'edit' && props.rowData) {
    ...
    // 如果有参数，将逗号分隔的字符串转为数组
    methodParamTags.value = model.value.methodParams ? model.value.methodParams.split(',') : [];
  }
}

// 实时更新 SPEL 表达式
function updateSpelExpression() {
  if (!model.value.componentName || !model.value.methodName) {
    model.value.viewSpel = '';
    return;
  }
  ...
  model.value.viewSpel = `#{@${model.value.componentName}.${model.value.methodName}(${params})}`;
}

// 监听组件名、方法名和参数变化
watch(() => model.value.componentName, updateSpelExpression);
watch(() => model.value.methodName, updateSpelExpression);
...
async function handleSubmit() {
  ...
  // 将参数标签数组转为逗号分隔的字符串
  model.value.methodParams = methodParamTags.value.join(',');

  const { id, componentName, methodName, methodParams, viewSpel, remark, status } = model.value;
